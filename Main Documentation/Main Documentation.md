...
validation_index <- createDataPartition(region$Day, p=0.80, list=FALSE)
# select 20% of the data for validation
validation <- region[-validation_index,]
# use the remaining 80% of data to training and testing the models
dataset <- region[validation_index,]
```
...
#Neural network
set.seed(7)
fit.nnet <- train(Day~., data=dataset, method="nnet", metric=metric, trControl=control)
# J48
set.seed(7)
fit.dt <- train(Day~., data=dataset, method="J48", metric=metric, trControl=control)
# kNN
set.seed(7)
fit.knn <- train(Day~., data=dataset, method="knn", metric=metric, trControl=control)
# NB
set.seed(7)
fit.nb <- train(Day~., data=dataset, method="nb", metric=metric, trControl=control)
```
    ...
    1. train(Day ~ ., data = dataset, method = "nnet", metric = metric, 
     .     trControl = control)

    2. train.formula(Day ~ ., data = dataset, method = "nnet", metric = metric, 
     .     trControl = control)
    ...
    5. stop(paste("Metric", metric, "not applicable for regression models"), 
     .     call. = FALSE)
